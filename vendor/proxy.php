...
if (! isset($MATOMO_URL)) {
    $MATOMO_URL = 'https://metrica.serveiscreatius.com/';
}

// Edit the line below, and replace xyz by the token_auth for the user "UserTrackingAPI"
// which you created when you followed instructions above.
if (! isset($TOKEN_AUTH)) {
    $TOKEN_AUTH = '85458302da220c3222a2765141ae9005';
}
...
if (strpos($path, 'piwik.php') === 0 || strpos($path, 'matomo.php') === 0) {
    ...
    if (empty($http_ip_forward_header)) {
        ...
        if ($isBulk) {
            ...
            if (isset($_GET['token_auth']) && $_GET['token_auth'] === '__TOKEN_AUTH__') { unset($_GET['token_auth']); }
            if (isset($_POST['token_auth']) && $_POST['token_auth'] === '__TOKEN_AUTH__') { unset($_POST['token_auth']); }
